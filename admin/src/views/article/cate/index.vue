...
const submitFormData = async (formEl) => {
  ...
  formEl.validate(async (valid) => {
    ...
    if (formData.value.id) {
      const res = await updateArticleCate(formData.value)
      if (res.code !== 200) return
      ElMessage.success('更新成功')
    } else {
      const res = await createArticleCate(formData.value)
      if (res.code !== 200) return
...
const showDialog = async (row) => {
  if (row.id) {
    const res = await getArticleCateDetail({ id: row.id })
    if (res.code !== 200) return
...
const delRow = async (row) => {
  ElMessageBox.confirm('此操作将永久删除该项数据, 是否继续?', '提示', {
    ...
    .then(async () => {
      const res = await deleteArticleCate({ id: row.id })
      if (res.code !== 200) return
...
const getTableData = async () => {
  const res = await getArticleCateList(queryInfo.value)
...
<template>
    ...
    <el-card shadow="never" class="query">
      <el-form :model="queryInfo" ref="queryFormRef" @submit.prevent>
        ...
        <el-form-item>
          ...
          <el-button :icon="Plus" type="primary" plain @click="showDialog">新建分类</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  ...
  <el-dialog :title="formData.id ? '编辑分类' : '新建分类'" v-model="dialogVisible" draggable>
