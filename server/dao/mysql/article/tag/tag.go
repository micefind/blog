...
func DeleteTag(req model.Tag) error {
	// 检查article表中是否存在关联记录
	result, err := mysql.CheckTableExist("article", "tag_id", req.Id)
	if err != nil {
		return err
	}
	if result {
		return errors.New("该标签下有文章，请先删除文章")
	}
...
func GetTagList(req model.Query) (model.List, error) {
	...
	// 构建查询条件（修改：使用tag表的name字段而非不存在的username）
	if req.Name != "" {
	...
	var tags []model.TagDetail = []model.TagDetail{}
	for rows.Next() {
